...
function BestAccountsBasedOnOrder({
  role,
  reportApiEndPoint = "",
  tableHeaderColumns = [],
}: props) {
  ...
  const fetchData = async () => {
    ...
    const fetchedDataToAccountData: AccountInfo[] = fetchedData.map(
      (account: any) => ({
  ...
  const sortData: AccountInfo[] = accountsList.sort(
  ...
  return (
    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-lg">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-gray-800">Best {role}</h1>
