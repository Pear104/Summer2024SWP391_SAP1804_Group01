...
import DeliveryColumnHeader from "./DeliveryColumnHeader";

export default function DeliveryStaffsManagement() {
  ...
  const [DeliveryList, DeliveryAccount] = useQueries({
    queries: [
      {
  ...
  const renderDeliveryRow = (order: any) => (
    <DeliveryDataRow key={order.orderId} order={order} />
  );
        ...
        <div className="flex justify-start space-x-1 items-center">
          <div className="self-center">
            <h1 className="text-2xl">
              {" "}
              Order List of {DeliveryAccount?.data?.name}
            </h1>
          </div>
        </div>
      </div>
          ...
          <div className="flex justify-between mb-1 items-center">
            <h3>
              ...
              >
                ...
                <Form.Item>
                  <DeliveryStatusFilter statusText={statusText} />
                </Form.Item>
              </Form>
            </h3>
              ...
              <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    ...
                    {DeliveryList?.data?.orders?.length > 0 ? (
                      DeliveryList.data.orders.map(renderDeliveryRow)
                    ) : (
                ...
                <div className="flex justify-center items-center px-8 py-4 bg-gray-100">
                  {DeliveryList?.data?.orders?.length > 0 && (
                    <Pagination
                      ...
                      defaultCurrent={
                        (DeliveryList?.data &&
                          DeliveryList?.data.currentPage.toString()) ||
                        "1"
                      }
                      total={
                        DeliveryList?.data && DeliveryList?.data.totalCount
                      }
