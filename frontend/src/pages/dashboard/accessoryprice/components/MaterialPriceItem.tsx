...
import { Button, Card, Input, Modal, Popover, message } from 'antd';
...
const MaterialPriceItem = ({ materialItem }: { materialItem: any }) => {
    ...
    const handleOk = async () => {
        ...
        if (parseFloat(newPrice) < 0 || parseFloat(newPrice) > 1000000000) {
            message.error('Please enter a valid material price between 0 and 1,000,000,000.');
            return;
        }
        setLoading(true);
        Modal.confirm({
            title: 'Are you sure you want to update this price for this kind of material?',
            onOk: async () => {
    ...
    if (materialItem.karat === 18) {
        description = '18k gold is an alloy consisting of 75% pure gold and 25% other metals. It offers a balance of durability and a rich gold color, making it ideal for jewelry. The added metals increase its strength and resistance to wear and tear, making it more suitable for everyday use.';
    } else if (materialItem.karat === 24) {
        description = '24k gold is 100% pure gold, known for its rich yellow hue and exceptional softness. This makes it highly desirable, but it is less suitable for jewelry due to its malleability and susceptibility to scratches and bending.';
    } else {
                ...
                <Meta
                    ...
                    description={
                        <>
                            ...
                            <div className='text-bold text-red-500 text-2xl'>Unit Price: {materialItem.unitPrice}</div>
                            <div>
